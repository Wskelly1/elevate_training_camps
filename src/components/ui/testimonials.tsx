...
const TestimonialCard = ({
	...
	useEffect(() => {
		...
		if (isExpanded) {
			// Hide navbar when modal is open by adding a CSS class to body
			document.body.classList.add('modal-open');

			// Also dispatch the custom event as backup
			const hideNavEvent = new CustomEvent('navVisibility', {
			...
			// Show navbar when modal is closed by removing the CSS class
			document.body.classList.remove('modal-open');

			// Also dispatch the custom event as backup
			const showNavEvent = new CustomEvent('navVisibility', {
				detail: { visible: true }
			});
		...
		return () => {
			// Cleanup: remove the CSS class when component unmounts
			document.body.classList.remove('modal-open');
